...
#import "SSMessageViewController.h"
...
@interface SSMessageViewController () <NSFetchedResultsControllerDelegate>
{
...
@implementation SSMessageViewController
...
- (void)fetchData
{
    ...
    [[[SSChatModule sharedInstance] xmppStream] sendElement:iQ];
}
...
- (void)didPressSendButton:(UIButton *)button
         ...
         senderDisplayName:(NSString *)senderDisplayName
                      date:(NSDate *)date
{
    [[SSChatModule sharedInstance] sendMessage:text toID:self.jidStr];
...
{
    SSMessage *message = items[indexPath.row];
...
{
    SSMessage *message = items[indexPath.row];
    SSMessageAvtarInfo *avatar = [SSMessageAvtarInfo avatarObjectForUsername:message.senderId];
    return avatar;
}
...
- (NSAttributedString *)collectionView:(JSQMessagesCollectionView *)collectionView attributedTextForMessageBubbleTopLabelAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.item - 1 > 0) {
        SSMessage *previousMessage = items[indexPath.item - 1];
...
- (UICollectionViewCell *)collectionView:(JSQMessagesCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SSMessage *msg = items[indexPath.row];
    
    if ([msg isKindOfClass:[SSMessage class]]) {
...
{
    SSMessage *currentMessage = items[indexPath.row];
    if ([[currentMessage senderId] isEqualToString:self.senderId]) {
        return 0.0f;
    }
    
    if (indexPath.item - 1 > 0) {
        SSMessage *previousMessage = items[indexPath.row - 1];
